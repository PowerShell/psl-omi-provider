...
#define LZ_WINDOW_SIZE      (1 << 13)
#define LZ_SMALL_LEN_BITS   3
#define LZ_MAX_SMALL_LEN    ((1 << LZ_SMALL_LEN_BITS) - 1)

#define Z_HASH_SIZE         (1 << 15)

#define RTL_XPRESS_TAG      'srpX'
...
typedef struct _XPRESS_LZ_WORKSPACE_MAX {
    PUCHAR HashTable[Z_HASH_SIZE];
    PUCHAR PrevMatch[LZ_WINDOW_SIZE * 2];
} XPRESS_LZ_WORKSPACE_MAX, *PXPRESS_LZ_WORKSPACE_MAX;
...
typedef struct _XPRESS_HUFF_WORKSPACE_MAX {
    PUCHAR HashTable[Z_HASH_SIZE];
    PUCHAR PrevMatch[HUFF_WINDOW_SIZE * 2];
    HUFFMAN_WORKSPACE Huffman;
    UCHAR LzPass[HUFFMAN_BLOCK_SIZE * 9 / 8 + 20];
} XPRESS_HUFF_WORKSPACE_MAX, *PXPRESS_HUFF_WORKSPACE_MAX;
...
RtlCompressBufferXpressLzStandard (
    ...
    IN PXPRESS_LZ_WORKSPACE Workspace,
...
RtlCompressBufferXpressLzMax (
    ...
    IN ULONG CompressedBufferSize,
    OUT PULONG FinalCompressedSize,
    IN PXPRESS_LZ_WORKSPACE_MAX Workspace,
    IN PRTL_XPRESS_CALLBACK_FUNCTION Callback,
    IN PVOID CallbackContext,
    IN ULONG ProgressBytes
    );
...
RtlCompressBufferXpressHuffMax (
    ...
    IN ULONG CompressedBufferSize,
    OUT PULONG FinalCompressedSize,
    IN PXPRESS_HUFF_WORKSPACE_MAX Workspace,
    IN PRTL_XPRESS_CALLBACK_FUNCTION Callback,
    IN PVOID CallbackContext,
    IN ULONG ProgressBytes
    );
...
//
// ARM intrinsics implementations.
//
...
extern const USHORT XpressHashFunction[3][256];
extern const UCHAR XpressHighBitIndexTable[256];

#if !defined(BLDR_KERNEL_RUNTIME)

NTSTATUS
RtlCompressBufferProgress (
    IN USHORT FormatEngine,
    IN PUCHAR UncompressedBuffer,
    IN ULONG UncompressedBufferSize,
    OUT PUCHAR CompressedBuffer,
    IN ULONG CompressedBufferSize,
...
Arguments:
    ...
    FormatEngine - Specifies the format and engine.  Available options:
...
Return Value:
    ...
    STATUS_UNSUPPORTED_COMPRESSION - An invalid FormatEngine value was used.

--*/

{
    PXPRESS_LZ_WORKSPACE StandardWorkspace;
    PXPRESS_LZ_WORKSPACE_MAX MaxWorkspace;
    PXPRESS_HUFF_WORKSPACE HuffStandardWorkspace;
    PXPRESS_HUFF_WORKSPACE_MAX HuffMaxWorkspace;
    PVOID AlignedWorkspace;
    USHORT Format;
    USHORT Engine;

    Format = FormatEngine & COMPRESSION_FORMAT_MASK;
    Engine = FormatEngine & COMPRESSION_ENGINE_MASK;

    AlignedWorkspace = ALIGN_UP_POINTER(WorkSpace, ULONG_PTR);

    if (Format == COMPRESSION_FORMAT_XPRESS) {

        if (Engine == COMPRESSION_ENGINE_STANDARD) {

            StandardWorkspace = (PXPRESS_LZ_WORKSPACE)AlignedWorkspace;

            return RtlCompressBufferXpressLzStandard(UncompressedBuffer,
            ...
            MaxWorkspace = (PXPRESS_LZ_WORKSPACE_MAX)AlignedWorkspace;

            return RtlCompressBufferXpressLzMax(UncompressedBuffer,
                                                UncompressedBufferSize,
                                                CompressedBuffer,
                                                CompressedBufferSize,
                                                FinalCompressedSize,
    ...
    } else if (Format == COMPRESSION_FORMAT_XPRESS_HUFF) {
        ...
        if (Engine == COMPRESSION_ENGINE_STANDARD) {
            ...
            HuffMaxWorkspace = (PXPRESS_HUFF_WORKSPACE_MAX)AlignedWorkspace;

            return RtlCompressBufferXpressHuffMax(UncompressedBuffer,
                                                  UncompressedBufferSize,
                                                  CompressedBuffer,
                                                  CompressedBufferSize,
                                                  FinalCompressedSize,
...
Arguments:
    ...
    FormatEngine - The format of the compressed data.  Possible values:
        COMPRESSION_FORMAT_XPRESS and COMPRESSION_FORMAT_XPRESS_HUFF
...
Return Value:
    ...
    STATUS_UNSUPPORTED_COMPRESSION - Invalid Format specified.
...

{
    ...
    if (Format == COMPRESSION_FORMAT_XPRESS) {
        ...
        return RtlDecompressBufferXpressLzProgress(UncompressedBuffer,
                                                   UncompressedBufferSize,
                                                   CompressedBuffer,
                                                   CompressedBufferSize,
        ...
        return RtlDecompressBufferXpressHuffProgress(UncompressedBuffer,
                                                     UncompressedBufferSize,
                                                     CompressedBuffer,
                                                     CompressedBufferSize,
                                                     FinalUncompressedSize,
...
Arguments:
    ...
    Engine - Standard or max.  Controls the speed vs. compression ratio
        trade-off.  This is the same "engine" value used by RtlCompressBuffer.

    CompressBufferWorkSpaceSize - Receives the size in bytes needed to compress
        a buffer.

    CompressFragmentWorkSpaceSize - Receives zero.

Return Value:

    STATUS_NOT_SUPPORTED - An unrecognized Engine value was used.
...

{
    if (Engine == COMPRESSION_ENGINE_STANDARD) {
        ...
        *CompressBufferWorkSpaceSize = sizeof(XPRESS_LZ_WORKSPACE) +
                                       sizeof(ULONG_PTR) - 1;
        ...
        *CompressBufferWorkSpaceSize = sizeof(XPRESS_LZ_WORKSPACE_MAX) +
                                       sizeof(ULONG_PTR) - 1;
...
Routine Description:
    ...
    This function uses a fast LZ77 method.
...

{
    ...
    //
    // Here is a description of the compression format: the compressed buffer
    // consists of literals, matches, and flags indicating whether each element
    // is a literal or a match.  Literals are individual bytes copied directly
    // from the uncompressed data.  Matches encode an offset and a length,
    // indicating that data should be copied from earlier in the buffer.  Flags
    // come in 32-bit chunks encoding the literal/match status of the next 32
    // elements (in the code, these flags are referred to as "tags").
    //
    ...
    // bounds checks.  The input bound is set up to let us look at 32 one-byte
    // literals + 6 bytes for the hash function.  The output bound lets us write
    // out 32 literals and one match.
    //

    SafeBufferEnd = InputBufferEnd - 38;
    ...
    ProgressMark = min(SafeBufferEnd, InputPos + ProgressBytes);
    ...
    //
    // Set up the tags, and write out the first byte as a literal.
    //
    ...
    for (i = 0; i < HASH_TABLE_SIZE; ++i) {
        Workspace->HashTable[i] = UncompressedBuffer;
    }
    for (i = 0; i < HASH_TABLE_EX_SIZE; ++i) {
        Workspace->HashTableEx[i] = UncompressedBuffer;
    }
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            //
            // Try to find a three-byte match.  And add our current position to
            // the hash table so we can find it later.
            //

            HashTableTemp = &Workspace->HashTable[InputPos[0]];
            ...
            assert(OutputPos < OutputBufferEnd);
            __analysis_assume(OutputPos < OutputBufferEnd);
            ...
            //
            // Put a '0' in the tags to indicate the literal, and write them out
            // if we filled it up.
            //
        ...
        if (InputPos[3] != Match[3]) {
            ...
            //
            // This match is only three bytes.  Try to find a four-byte match.
            //

            ExHashValue = HashValue * 2 + InputPos[0];

            MatchEx = Workspace->HashTableEx[ExHashValue + InputPos[3]];
            Workspace->HashTableEx[ExHashValue + Match[3]] = Match;

            if (InputPos - MatchEx < LZ_WINDOW_SIZE &&
                *((ULONG UNALIGNED *)InputPos) == *((ULONG UNALIGNED *)MatchEx))
            {
                Workspace->HashTableEx[ExHashValue + InputPos[3]] = InputPos;
                Match = MatchEx;
            } else {
                InputPos += 3;
                Match += 3;
        ...
        //
        // Keep trying to extend the match.
        //

        if (InputPos[4] != Match[4]) {

            ExHashValue = _rotl8(InputPos[0] ^
                                 _rotr8(InputPos[2] + InputPos[1] - 159, 1),
                                 3) * HASH_COEFF_HIGH
                          + _rotr8(InputPos[1] ^
                                   _rotl8(InputPos[3] + InputPos[0], 3),
                                   1) * HASH_COEFF_LOW;

            MatchEx = Workspace->HashTableEx[ExHashValue + InputPos[4]];
            Workspace->HashTableEx[ExHashValue + Match[4]] = Match;
            ...
            if (InputPos - MatchEx < LZ_WINDOW_SIZE &&
                *((ULONG UNALIGNED *)InputPos) == *((ULONG UNALIGNED *)MatchEx) &&
                ...
                InputPos != MatchEx)
            {
                Workspace->HashTableEx[ExHashValue + InputPos[4]] = InputPos;
                Match = MatchEx;
            } else {
                InputPos += 4;
                Match += 4;
        ...
        if (InputPos[5] != Match[5]) {
            ...
            MatchEx = Workspace->HashTableEx[ExHashValue + InputPos[5]];
            Workspace->HashTableEx[ExHashValue + Match[5]] = Match;
            ...
            if (InputPos - MatchEx < LZ_WINDOW_SIZE &&
                *((ULONG UNALIGNED *)InputPos) == *((ULONG UNALIGNED *)MatchEx) &&
                ...
                InputPos != MatchEx)
            {
                Workspace->HashTableEx[ExHashValue + InputPos[5]] = InputPos;
                Match = MatchEx;
            } else {
                InputPos += 5;
                Match += 5;
        ...
        //
        // At this point, the match is at least 6-bytes long.  See how long it
        // extends.
        //
        ...
        //
        // InputPos < SafeBufferEnd implies InputPos + 32 < InputBufferEnd
        //
    ...
    MatchDone:
        ...
        assert(MatchOffset <= LZ_WINDOW_SIZE);

        //
        // Encode the match.  Typically, this will require two bytes.  The
        // offset always takes up 13 bits, and the length gets at least 3 bits.
        //
        ...
        //
        // Shift the offset to make room for the length.
        //

        MatchOffset <<= LZ_SMALL_LEN_BITS;
        ...
        if (MatchLen < LZ_MAX_SMALL_LEN) {
            ...
            *((USHORT UNALIGNED *)OutputPos) = (USHORT)MatchOffset;
            ...
            //
            // The length does not fit in 3 bits.  Record a special value to
            ...
            *((USHORT UNALIGNED *)OutputPos) = (USHORT)MatchOffset;
            ...
            if (LastLenHalfByte == 0) {
                ...
                if (MatchLen < 15) {
                    ...
                    //
                    // We need even more bits to encode the length.
                    //
                ...
                EncodeExtraLen:
                    ...
                    if (MatchLen < 255) {
                        ...
                        if (MatchLen < (1 << 16)) {
                            ...
                            *((USHORT UNALIGNED *)OutputPos) = (USHORT)MatchLen;
                            OutputPos += 2;
        ...
        //
        // Record a '1' in the tags to indicate a match.
        //
 ...
 SafeDone:
    ...
    //
    // We're close to the end of either the input buffer or the output buffer.
    // Write out the remaining data as literals.
    //
    ...
    if (OutputPos >= OutputBufferEnd) {
        ...
        //
        // We reached the end of the output buffer.  Return an error.
        //
    ...
    //
    // Shift the remaining tags to be consistent with the other tags.
    //
    ...
    *TagsOut = RunningTags;
    ...
    //
    // Return the compressed size to the caller.
    //
    ...
    *FinalCompressedSize = (ULONG)(OutputPos - CompressedBuffer);
    ...
    if (*FinalCompressedSize < 8) {
        ...
        //
        // The old version of Xpress refuses to decompress a buffer smaller than
        // 8 bytes.
        //
...

{
    ...
    if (UncompressedBuffer <= (PUCHAR)(LZ_WINDOW_SIZE + 1)) {
    ...
    memset(&Workspace->HashTable, 0, sizeof(Workspace->HashTable));
    ...
    for (;;) {
        ...
        ProgressMark = min(HashBound, InputPos + ProgressBytes);
        ...
        while (HashPos < HashBound) {
            ...
            assert(HashWindowIndex < LZ_WINDOW_SIZE * 2);

            Workspace->PrevMatch[HashWindowIndex] =
                Workspace->HashTable[HashValue];

            Workspace->HashTable[HashValue] = HashPos;

            ++HashPos;
            ++HashWindowIndex;
        }

        PrevMatchList = &Workspace->PrevMatch[0];
        ...
        for (;;) {
            ...
            for (;;) {
                ...
                if (Next4 == *((ULONG UNALIGNED *)Match)) {
...
Routine Description:
    ...
    This function uses an LZ77 method.
...
Arguments:
    ...
    WorkSpace - A buffer for compression data structures.  Retrieve the
        necessary size by calling RtlGetCompressionWorkSpaceSize with the same
        Engine parameter.
...

{
    ...
    UNREFERENCED_PARAMETER(UncompressedChunkSize);

    //
    // If the given WorkSpace is not pointer-aligned, make it so.
    //
    ...
    if (Engine == COMPRESSION_ENGINE_MAXIMUM) {
...

{
    ...
    UNREFERENCED_PARAMETER(UncompressedChunkSize);
    UNREFERENCED_PARAMETER(WorkspacePvoid);
    ...
    for (;;) {
        ...
        InputPos += 2;
        MatchLen = MatchOffset & LZ_MAX_SMALL_LEN;
            ...
            *((ULONG UNALIGNED *)(OutputPos+4)) = *((ULONG UNALIGNED *)(MatchSrc+4));
            ...
            for (;;) {
                ...
                if (OutputPos >= SafeOutputEnd) {
                    ...
                    __movsb(OutputPos, MatchSrc, MatchLen);
                    OutputPos += MatchLen;
...

{
    ...
    ProgressOutputMark = min(SafeOutputEnd, OutputPos + ProgressBytes);
    ...
    for (;;) {
        ...
        GetNextTags:
            ...
            if (InputPos >= SafeInputEnd) {
                goto SafeDecodeForInputNewTagEntry;
            }
        ...
        if (MatchLen == LZ_MAX_SMALL_LEN) {
            ...
            if (MatchLen == (1 << 4) - 1) {
                ...
                if (InputPos + 7 >= SafeInputEnd) {
                    goto SafeDecodeForInputLongLen;
                }
    ...
    //
    // The next loop is the "safe for input" mode.  In this mode, we're near the
    // end of the input buffer, but we're not near the end of the output buffer.
    //
    ...
    for (;;) {
        ...
        ProcessLiteralSafeForInput:
            ...
            *((ULONG UNALIGNED *)OutputPos) = *((ULONG UNALIGNED *)InputPos);
            ...
            goto ProcessLiteralSafeForInput;
        }
...
Arguments:
    ...
    CompressFragmentWorkSpaceSize - Receives the size in bytes need to
        decompress a buffer.  Fragment decompression is not supported.
...

{
    if (Engine == COMPRESSION_ENGINE_STANDARD) {
        ...
        *CompressFragmentWorkSpaceSize = sizeof(XPRESS_HUFF_DECODE_WORKSPACE) +
                                         sizeof(ULONG_PTR) - 1;
        ...
        *CompressBufferWorkSpaceSize = sizeof(XPRESS_HUFF_WORKSPACE_MAX) +
                                       sizeof(ULONG_PTR) - 1;
...
Routine Description:
    ...
    This routine is called during hibernate by the power manager to mark all
    pages for the boot phase of hibernate that might be accessed when
    processing the kernel phase of resume.
...

{
    ...
    PoSetHiberRange(NULL,
                    PO_MEM_BOOT_PHASE,
                    (PVOID)XpressHashFunction,
                    sizeof(XpressHashFunction),
                    RTL_XPRESS_TAG);

    PoSetHiberRange(NULL,
                    PO_MEM_BOOT_PHASE,
                    (PVOID)XpressHighBitIndexTable,
                    sizeof(XpressHighBitIndexTable),
                    RTL_XPRESS_TAG);
...
Routine Description:
    ...
    This function uses a slow LZ77+Huffman method.
...

{
    ...
    // RtlCompressBufferXpressHuffStandard and RtlCompressBufferXpressLzMax.
    // Many of the comments in those functions apply here, so they will not be
    // repeated.
    //
    ...
    if (UncompressedBuffer <= (PUCHAR)(HUFF_WINDOW_SIZE + 1)) {
    ...
    for (;;) {
        ...
        SafeHuffBlockEnd = HuffBlockEnd - 5;
        ...
        //
        // Pre-hash the data up the end of the Huffman block.  Fortunately, the
        // Huffman block size is the same as the window size.
        //

        HashPos = InputPos;
        HashWindowIndex = (HashPos - UncompressedBuffer) %
                          (HUFF_WINDOW_SIZE * 2);

        while (HashPos < SafeHuffBlockEnd) {

            HashValue = XpressHashFunction[0][HashPos[0]];
            HashValue ^= XpressHashFunction[1][HashPos[1]];
            HashValue ^= XpressHashFunction[2][HashPos[2]];

            Match = Workspace->HashTable[HashValue];
            Workspace->HashTable[HashValue] = HashPos;

            Workspace->PrevMatch[HashWindowIndex] = Match;

            ++HashPos;
            HashWindowIndex = (HashWindowIndex + 1) % (HUFF_WINDOW_SIZE * 2);
        }
        ...
        for (;;) {
            ...
            for (;;) {
                ...
                Match = Workspace->PrevMatch[(InputPos - UncompressedBuffer) %
                                             (HUFF_WINDOW_SIZE * 2)];

                Next4 = *((ULONG UNALIGNED *)InputPos);

                //
                // One detail worth noting here is this: with the "LZ" encoding,
                // we can encode an offset that is equal to the window size.
                // But with the Huffman format, we cannot do this.  So the line
                //
                //   if (Match + LZ_WINDOW_SIZE >= InputPos)   <-- note >=
                //
                // is changed to
                //
                //   if (Match + HUFF_WINDOW_SIZE > InputPos) <-- == not allowed
                //

                if (Match + HUFF_WINDOW_SIZE > InputPos) {

                    Xor4 = Next4 ^ *((ULONG UNALIGNED *)Match);

                    if (Xor4 == 0) {
                        SavedInputPos = InputPos;
                        WindowBound = InputPos - HUFF_WINDOW_SIZE;
                        goto ExtendMatch;
                    } else if ((Xor4 & 0xffffff) == 0) {
                        break;
                    }

                    Match = Workspace->PrevMatch[(Match - UncompressedBuffer) %
                                                 (HUFF_WINDOW_SIZE * 2)];
            ...
            EncodeLiteral:
                ...
                ++Workspace->Huffman.Frequencies[(UCHAR)Next4];
                OutputPos[0] = (UCHAR)Next4;
                ++OutputPos;
                ++InputPos;
            ...
            for (;;) {
                ...
                MatchDone:
                    ...
                    if (MatchLen > BestMatchLen) {
                        ...
                        if (Match + MatchLen > SavedInputPos) {
                            break;
                        }
            ...
            InputPos = SavedInputPos + MatchLen;

            assert(BestMatch < SavedInputPos);

            MatchOffset = SavedInputPos - BestMatch;
            ...
            *((USHORT UNALIGNED *)OutputPos) = (USHORT)MatchOffset;
            ...
            //
            // No need to check against the OutputPos boundary because the
            // LzPass array is big enough.
            //
        }
...
Routine Description:
    ...
    This function uses an LZ77+Huffman method.
...
{
    PXPRESS_HUFF_WORKSPACE StandardWorkspace;
    PXPRESS_HUFF_WORKSPACE_MAX MaxWorkspace;
    PVOID AlignedWorkspace;
    ...
    if (Engine == COMPRESSION_ENGINE_STANDARD) {
        ...
        StandardWorkspace = (PXPRESS_HUFF_WORKSPACE)AlignedWorkspace;
    ...
    if (Engine == COMPRESSION_ENGINE_MAXIMUM) {
        ...
        MaxWorkspace = (PXPRESS_HUFF_WORKSPACE_MAX)AlignedWorkspace;
